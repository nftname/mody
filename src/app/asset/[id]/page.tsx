...
// OpenSea-like Dark Background
const OPENSEA_BG = '#04111d'; 
const CARD_BG = '#202225';
// Real Gold Ingot Gradient
    ...
    return (
        <div onClick={onClose} style={{
            ...
            <div onClick={(e) => e.stopPropagation()} style={{
                ...
                <h4 className="text-white fw-bold mb-3" style={{ fontSize: '20px' }}>{title}</h4>
                <p className="text-secondary mb-4" style={{ fontSize: '15px', lineHeight: '1.5' }}>{message}</p>
                <button onClick={onClose} className="btn w-100 fw-bold text-dark" style={{ background: BTN_GRADIENT, border: 'none', padding: '12px', borderRadius: '10px' }}>
...
// Fixed: Added 'shadow' property to match the interface expected in JSX
const getHeroStyles = (tier: string) => {
    switch(tier?.toLowerCase()) {
        case 'immortal': return { 
            ...
            textColor: '#FCD535',
            shadow: '0 0 40px rgba(252, 213, 53, 0.1)'
        };
        case 'elite': return { 
            bg: 'linear-gradient(135deg, #1a0505 0%, #2a0a0a 100%)', 
            border: '1px solid #ff4d4d', 
            textColor: '#ff4d4d',
            shadow: '0 0 40px rgba(255, 77, 77, 0.1)'
        };
        default: return { 
            bg: '#262b2f', 
...
function AssetPage() {
    ...
    // Fixed: Added guard clause to prevent "undefined" error
    ...
    if (loading) return <div className="vh-100 d-flex justify-content-center align-items-center text-secondary" style={{ backgroundColor: OPENSEA_BG }}>Loading Asset...</div>;
    if (!asset) return <div className="vh-100 d-flex justify-content-center align-items-center text-white" style={{ backgroundColor: OPENSEA_BG }}>Asset Not Found</div>;
    
    const style = getHeroStyles(asset.tier);
    
    // --- SMART CALCULATION LOGIC ---
    const targetAmount = offerPrice ? Number(offerPrice) : 0;
    const missingAmount = Math.max(0, targetAmount - wpolBalance);
    const safeBuffer = missingAmount > 0 ? (missingAmount * 1.01).toFixed(4) : "0";

    const needsFunds = missingAmount > 0;
    const needsApproval = !needsFunds && wpolAllowance < targetAmount;

    return (
        <main style={{ backgroundColor: OPENSEA_BG, minHeight: '100vh', paddingBottom: '80px', fontFamily: 'sans-serif' }}>
            ...
            {showPayEmbed && (
                <div style={{
                    ...
                    <div style={{ position: 'relative', width: '100%', maxWidth: '420px', padding: '10px' }}>
                        <button 
                            onClick={() => { setShowPayEmbed(false); refreshWpolData(); }} 
                            style={{ position: 'absolute', top: '-40px', right: '0', background: 'transparent', border: 'none', color: '#fff', fontSize: '20px', cursor: 'pointer' }}
                        >
            ...
            <div className="container py-4">
                ...
                <div className="row g-5">
                    ...
                    <div className="col-lg-5">
                         <div className="rounded-4 d-flex justify-content-center align-items-center position-relative overflow-hidden" style={{ background: CARD_BG, border: '1px solid #4c505c', minHeight: '500px' }}>
                        ...
                        <div className="mt-4 p-4 rounded-3" style={{ backgroundColor: CARD_BG, border: '1px solid #4c505c' }}>
                    ...
                    <div className="col-lg-7">
                        <div className="mb-3">
                            ...
                            <span className="text-secondary small">Owned by <span className="text-warning">{asset.owner.slice(0,6)}...{asset.owner.slice(-4)}</span></span>
                        </div>

                        <div className="p-4 rounded-3 mt-3 mb-4" style={{ backgroundColor: CARD_BG, border: '1px solid #4c505c' }}>
                            ...
                            {!account ? (
                                <button onClick={handleConnect} className="btn w-100 fw-bold text-dark" style={{ background: BTN_GRADIENT, border: 'none', height: '50px' }}>
                                    Connect Wallet
                                </button>
                            ) : (
                                    ...
                                    {listing ? (
                                        !isOwner ? (
                                            !isOfferMode ? (
                                                <div className="d-flex gap-3">
                                                    <TransactionButton
                                                        ...
                                                        onTransactionConfirmed={() => showModal('success', 'Success', 'Asset Purchased!')}
                                                        onError={() => showModal('error', 'Error', 'Transaction Failed')}
                                                        style={{ background: BTN_GRADIENT, color: '#000', fontWeight: 'bold', flex: 1, height: '50px', border: 'none' }}
                                                    >
                                                        Buy Now
                                                    </TransactionButton>
                                                    <button onClick={() => setIsOfferMode(true)} className="btn fw-bold flex-grow-1 text-white" style={{ background: 'transparent', border: '1px solid #ffffff', height: '50px' }}>
                                                        Make Offer
                                                    </button>
                                                </div>
                                            ) : (
                                                <div className="p-3 rounded bg-dark border border-secondary">
                                                    ...
                                                    {needsFunds ? (
                                                        <button 
                                                            ...
                                                            style={{ background: BTN_GRADIENT, border: 'none', padding: '12px' }}
                                                        >
                                                            Get {safeBuffer} WPOL to Continue
                                                        </button>
                                                    ) : needsApproval ? (
                                                        <TransactionButton
                                                            transaction={handleApprove}
                                                            onTransactionConfirmed={() => refreshWpolData()}
                                                            style={{ width: '100%', background: BTN_GRADIENT, color: '#000', border: 'none', fontWeight: 'bold' }}
                                                        >
                                                            Approve WPOL Access
                                                        </TransactionButton>
                                                    ) : (
                                                        <TransactionButton
                                                            ...
                                                            onTransactionConfirmed={() => { showModal('success', 'Offer Sent', 'Your offer is active!'); setIsOfferMode(false); }}
                                                            onError={() => showModal('error', 'Error', 'Offer failed.')}
                                                            style={{ width: '100%', background: BTN_GRADIENT, color: '#000', border: 'none', fontWeight: 'bold' }}
                                                        >
                                                            Confirm Offer
                                                        </TransactionButton>
                                            ...
                                            <TransactionButton 
                                                ...
                                                style={{ width: '100%', background: '#333', color: '#fff' }}
                                            >
                                                Cancel Listing
                                            </TransactionButton>
                                        )
                                        ...
                                        isOwner ? (
                                            !isListingMode ? (
                                                <button onClick={() => setIsListingMode(true)} className="btn w-100 fw-bold text-dark" style={{ background: BTN_GRADIENT, height: '50px' }}>List Item</button>
                                            ) : (
                                                <div className="d-flex flex-column gap-2">
                                                    <input type="number" className="form-control bg-dark text-white" placeholder="Price (POL)" value={sellPrice} onChange={(e) => setSellPrice(e.target.value)} />
                                                    <div className="d-flex gap-2">
                                                    {!isApproved ? (
                                                        <TransactionButton transaction={() => setApprovalForAll({ contract: nftContract, operator: MARKETPLACE_ADDRESS, approved: true })} onTransactionConfirmed={() => setIsApproved(true)} style={{ flex: 1, background: '#fff', color: '#000' }}>Approve</TransactionButton>
                                                    ) : (
                                                        <TransactionButton 
                                                            transaction={() => {
                                                            ...
                                                            onTransactionConfirmed={() => showModal('success', 'Listed', 'Asset Listed')}
                                                            style={{ flex: 1, background: BTN_GRADIENT, color: '#000' }}
                                                        >
                                                            Confirm
                                                        </TransactionButton>
                                                    )}
                                            ...
                                            <button disabled className="btn w-100 disabled" style={{ background: '#333', color: '#666' }}>Not Listed</button>
                                        )
                                    )}
                                </div>
                            )}
                        ...
                        <div className="mb-4">
                            ...
                            <div className="rounded-3 p-3" style={{ backgroundColor: CARD_BG, border: '1px solid #4c505c', height: '250px' }}>
                        ...
                        <div className="mb-5">
                            ...
                            <div className="rounded-3 overflow-hidden" style={{ border: '1px solid #4c505c' }}>
                                    ...
                                    <tbody>
                                        {offersList.length > 0 ? (
                                            offersList.map((offer, index) => (
                                                <tr key={index}>
                                                    <td className="ps-3 fw-bold text-white">{offer.totalOfferAmount} WPOL</td>
                                                    <td className="text-warning">{offer.offeror.slice(0,6)}...{offer.offeror.slice(-4)}</td>
