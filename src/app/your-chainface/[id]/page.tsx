    ...
    <main style={{ backgroundColor: '#F0EDF2', minHeight: '100vh', fontFamily: '"Inter", sans-serif', position: 'relative', zIndex: 1000 }}>
      ...
      <div className="page-container">
          ...
          <div style={{ maxWidth: '800px', margin: '20px auto', textAlign: 'center', padding: '0 20px' }}>
              ...
              <div className="pay-grid">
                  {/* أزرار ديناميكية: تظهر فقط إذا كان العنوان موجوداً في الداتا بيز */}
                  {profileData.wallets.btc && <Web3PaymentButton type="BTC" name="Bitcoin" address={profileData.wallets.btc} onClick={() => handleCopyWallet(profileData.wallets.btc, 'BTC')} />}
                  {profileData.wallets.eth && <Web3PaymentButton type="ETH" name="Ethereum" address={profileData.wallets.eth} onClick={() => handleCopyWallet(profileData.wallets.eth, 'ETH')} />}
                  {profileData.wallets.matic && <Web3PaymentButton type="POLYGON" name="Polygon" address={profileData.wallets.matic} onClick={() => handleCopyWallet(profileData.wallets.matic, 'POL')} />}
                  {profileData.wallets.sol && <Web3PaymentButton type="SOL" name="Solana" address={profileData.wallets.sol} onClick={() => handleCopyWallet(profileData.wallets.sol, 'SOL')} />}
                  {profileData.wallets.bnb && <Web3PaymentButton type="BNB" name="BNB Chain" address={profileData.wallets.bnb} onClick={() => handleCopyWallet(profileData.wallets.bnb, 'BNB')} />}
                  {profileData.wallets.usdt && <Web3PaymentButton type="USDT" name="Tether" address={profileData.wallets.usdt} onClick={() => handleCopyWallet(profileData.wallets.usdt, 'USDT')} />}
                  
                  {/* إذا لم يضع أي محفظة، يظهر زر عام يشجع على إعداد المحفظة أو التبرع العام */}
                  {!profileData.wallets.btc && !profileData.wallets.eth && (
                      <Web3PaymentButton type="WALLET" name="General Wallet" address={profileData.owner} onClick={() => handleCopyWallet(profileData.owner, 'Owner Wallet')} />
                  )}
              </div>
      ...
      <div style={{ padding: '30px 20px', backgroundColor: '#fff', borderTop: '1px solid #eee', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: '20px' }}>
              {/* Dynamic ChainFace Button at Footer */}
