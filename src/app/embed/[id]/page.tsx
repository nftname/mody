...
const FullBarWidget = ({ theme }: { theme: 'dark' | 'light' }) => (
    <div className="d-flex gap-2 align-items-center justify-content-center w-100 h-100" 
        ...
        <NFXWidget theme={theme} />
        <NFXCapWidget theme={theme} />
        <NFXVolumeWidget theme={theme} />
    </div>
);
...
export default function EmbedPage() {
  ...
  const renderWidget = () => {
    switch (id) {
      case 'nfx-sentiment': return <NFXWidget theme={theme} />;
      case 'nfx-cap': return <NFXCapWidget theme={theme} />;
      case 'nfx-volume': return <NFXVolumeWidget theme={theme} />;
      case 'nfx-full-bar': return <FullBarWidget theme={theme} />;
  ...
  return (
    <div style={{ width: '100%', height: '100vh', backgroundColor: bgColor, overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <a href="https://nftnnm.com/nfx" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', display: 'block', transform: 'scale(0.9)' }}>
          {renderWidget()}
      </a>

      <a href="https://nftnnm.com/nfx" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', marginTop: '-5px' }}>
