...
// --- تصحيح الفلاتر الزمنية (The Logic Fix) ---
// هنا نحدد "كم يوم" نعود للوراء بناءً على الزر المختار
const TIMEFRAMES = [
    { label: '1H', value: '1H', days: 1 },    // يعرض آخر 24 ساعة (تفاصيل دقيقة)
    { label: '4H', value: '4H', days: 7 },    // يعرض آخر أسبوع
    { label: '1D', value: '1D', days: 30 },   // يعرض آخر شهر
    { label: '1W', value: '1W', days: 90 },   // يعرض آخر 3 شهور
    { label: '1M', value: '1M', days: 180 },  // يعرض آخر 6 شهور
    { label: '1Y', value: '1Y', days: 365 },  // يعرض آخر سنة
    { label: 'ALL', value: 'ALL', days: 0 }   // يعرض كل شيء (بدون حد)
];
...
export default function NGXLiveChart() {
  ...
  const [activeTimeframe, setActiveTimeframe] = useState('1D'); // الافتراضي يومي
  ...
  const fetchHistory = async (sectorKey: string) => {
    ...
    try {
        const { data: sectorData, error } = await supabase
            ...
            .limit(10000); // جلب كل التاريخ المتاح
        ...
        // قلب البيانات لتصبح (قديم -> حديث) للرسم
  ...
  useEffect(() => {
    if (seriesInstance && chartInstance && chartData.length > 0) {
        ...
        // إذا كان days > 0، نقوم بقص البيانات لعرض الفترة المحددة فقط
        if (tf && tf.days > 0) {
            const cutoffTime = Math.floor(Date.now() / 1000) - (tf.days * 24 * 60 * 60);
            filteredData = chartData.filter((d: any) => d.time >= cutoffTime);
        }
        // إذا كان days == 0 (مثل ALL)، نعرض filteredData (وهي كل البيانات)

        seriesInstance.setData(filteredData);
        chartInstance.timeScale().fitContent(); // ضبط الزوم تلقائياً على البيانات المعروضة
    }
