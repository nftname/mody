...
export default function NGXLiveChart() {
  ...
  // --- دالة جلب البيانات من الجدول الجديد (Clean Data) ---
  const fetchHistory = async (sectorKey: string) => {
    ...
    try {
        // جلب البيانات من الجدول الجديد ngx_volume_index
        const { data: sectorData, error } = await supabase
            ...
            .order('timestamp', { ascending: true }); // ترتيب زمني صحيح

        if (error) throw error;

        // تنسيق البيانات للرسم البياني
        const cleanData = sectorData.map((row: any) => ({
            time: Number(row.timestamp),
            value: Number(row.index_value)
        }));

        // إزالة التكرار إن وجد
        const uniqueData = cleanData.filter((v, i, a) => i === a.findIndex(t => t.time === v.time));
  ...
  // --- إعداد الرسم البياني (نفس التصميم القديم) ---
  useEffect(() => {
  ...
  // تحديث الرسم عند تغير البيانات أو الفلتر الزمني
  useEffect(() => {
  ...
  // تحديث تلقائي صامت كل دقيقة (Live Update)
  useEffect(() => {
      const interval = setInterval(() => {
          // في النسخة الجديدة، نعيد جلب البيانات من الداتا بيز فقط
          // لأن الـ Cron Job هو المسؤول عن الحسابات
          fetchHistory(activeSector);
      }, 60000); 
  ...
  return (
    <div className="ngx-chart-glass mb-4">
      ...
      <style jsx>{`
        /* كود CSS الأصلي مع إضافة إخفاء اللوجو */
        .ngx-chart-glass {
        ...
        /* كود سحري لإخفاء شعار تريدنج فيو */
